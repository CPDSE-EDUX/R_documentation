...
```{webr}
read_lines("data/dissolution_data_raw.csv", n_max = 3)
```
:::
...
In practice you pass a file path. This file starts with 2 lines of instrument metadata, so we also set `skip = 2` — see the *Skip metadata rows* example below for what happens without it.

```{webr}
library(tidyverse)

data <- read_csv("data/dissolution_data_raw.csv", skip = 2)
...
<div class="example-body">
The [example file](../data/dissolution_data_raw.csv) above has missing absorbance readings encoded three different ways — `"N/A"`, `"-"`, and a blank cell. Use the optional argument `na` to unify how all of them are handled, so they all become `NA`.

```{webr}
library(tidyverse)

df_na <- read_csv(
  "data/dissolution_data_raw.csv",
  skip = 2,
  na = c("", "N/A", "-")
)
...
<div class="example-body">
The [example file](../data/dissolution_data_raw.csv) also has a fully blank row between tablet 1 and tablet 2 — common after copy-pasting from Excel. By default `skip_empty_rows = TRUE` drops it automatically. Set it to `FALSE` if you need to keep it (it shows up as a row of `NA`s).

```{webr}
library(tidyverse)

read_csv("data/dissolution_data_raw.csv", skip = 2)
read_csv("data/dissolution_data_raw.csv", skip = 2, skip_empty_rows = FALSE)
```
</div>
</details>
...
<div class="example-body">
The [example file](../data/dissolution_data_raw.csv) starts with 2 lines of instrument metadata (device name, run date) before the real header row. Use `skip` to jump straight to the data — without it, `read_csv()` would try to use the device name as your column names.

```{webr}
library(tidyverse)

# without skip — the metadata line becomes a bogus header
read_csv("data/dissolution_data_raw.csv")
...
<div class="example-body">
If a file has no header row, or you want clearer column names than the original file provides, supply a character vector to `col_names`. Make sure the order matches the columns in the file, and use `skip` to drop everything above the data — here `skip = 3` removes the 2 metadata lines *and* the original header row, so they aren't read as data.
