...
<a class="source-link" href="/assets/data/dissolution_data_raw.csv" download>Example File (.csv) ↓</a>
</div>
...
<div class="example-body">
The [example file](/assets/data/dissolution_data_raw.csv) above has missing absorbance readings encoded three different ways — `"N/A"`, `"-"`, and a blank cell. Use the optional argument `na` to unify how all of them are handled, so they all become `NA`.

```{webr-r}
library(readr)

df_na <- read_csv(
  "/assets/data/dissolution_data_raw.csv",
  skip = 2,
  na = c("", "N/A", "-")
)
...
<div class="example-body">
The [example file](/assets/data/dissolution_data_raw.csv) starts with 2 lines of instrument metadata (device name, run date) before the real header row. Use `skip` to jump straight to the data — without it, `read_csv()` would try to use the device name as your column names.

```{webr-r}
library(readr)

# without skip — the metadata line becomes a bogus header
read_csv("/assets/data/dissolution_data_raw.csv")
